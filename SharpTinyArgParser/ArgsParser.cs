    ...
    public class ArgsParser
    {
        ...
        protected static void ParseArgsSetValue(object obj, PropertyInfo propertyInfo, string value)
        {
            ...
            if (propertyType.IsPrimitive)
            {
                ...
                valueObj = methodInfo.Invoke(null, new object[] { value });
            }
        ...
        public static T ParseArgs<T>(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                if (currentArg.StartsWith("-"))
                {
                    ...
                    propertyInfoDict.TryGetValue(currentArgName, out propertyInfo);
                    ...
                    if (propertyInfo != null )
                    {
                        if ((i + 1 < args.Length))
                        {
                            ...
                            {
                                currentArgValue = currentArgValue.ToLower();
                                if (currentArgValue.StartsWith("-"))
                                {
                                    i--;
                                    currentArgValue = "true";
                                }
                            }
        ...
        public static string PrintHelp(Type type, string head, string appName) {
            return PrintHelp(type, head, appName,new string[0]);
        }
        public static string PrintHelp(Type type, string head, string appName, string[] examples)
        {
            ...
            if (propertyInfoList.Count > requiredPropertyList.Count && propertyInfoList.Count > 0)
            {
